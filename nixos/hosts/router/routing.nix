      ...
      ruleset = ''
        ...
        table netdev filter {
          chain ingress {
            ...
            ip saddr ${addresses.lan.ipv4.addr} log prefix "NFT_SPOOF_LAN: " group 2 drop 
            ip6 saddr ${addresses.lan.ipv6.addr} log prefix "NFT_SPOOF_LAN6: " group 2 drop 

            # drop incoming packets on WAN that are pretending to be internal ULA traffic
            ip6 saddr ${addresses.lan.ipv6.subnet} log prefix "NFT_DROP_SPOOF_ULA: " group 2 drop 

            # drop packets if the source address is unroutable ("how did you get here?")
            fib saddr . iif oif missing log prefix "NFT_DROP_UNROUTABLE: " group 3 drop 
          }
          chain ingress_wan {
            type filter hook ingress device ${interfaces.wan.name} priority -500;
            ip protocol icmp limit rate 5/second accept
            ip protocol icmp counter log prefix "NFT_DROP_ICMP: " group 4 drop 
            ...
            ip6 nexthdr icmpv6 counter log prefix "NFT_DROP_ICMP6: " group 4 drop 

            # drop incoming packets that are not locally addressed
            fib daddr . iif type != local log prefix "NFT_DROP_UNROUTABLE_WAN: " group 3 drop 
          }
        }

        table inet global {
          chain inbound_wan {
            ct state new log prefix "NFT_INBOUND_NEW_CONN: " group 5 limit rate 5/second
          ...
          chain forward {
            ...
            ip saddr 192.168.3.0/24 oifname wan0 ct state new log prefix "NFT_FORWARD_NEW_CONN: " group 6 limit rate 5/second
          ...
          chain postrouting {
            ...
            ip saddr ${addresses.lan.ipv4.subnet} oifname ${interfaces.wan.name} log prefix "NFT_NAT: " group 7 masquerade 
          }
        }
      '';
    };
